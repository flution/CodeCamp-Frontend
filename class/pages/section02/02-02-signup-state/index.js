...
export default function SignupStatePage() {
    ...
    function onCllckSignup(event){
        console.log(email) // 진짜 포장이 잘 되었는지 확인하기
        console.log(password) // 진짜 포장이 잘 되었는지 확인하기

        // 1. 검증하기
        if(email.includes("@") === false ) {
        // alert("이메일이 올바르지 않습니다.")
        // document.getElementById.apply("myerror").innerText = "이메일이 올바르지 않습니다." => 옛날 방식임
    ...
    return (
        <div>
            ...
            {/* <div id ="myerror"></div> ===> 옛날 방식임 */ }
            <div>{emailError}</div>
            비밀번호 : <input type="password" onChange={onChangePassword} />
            <button onClick={onCllckSignup}>회원가입</button>
        </div>
    )
